...
export default function FormReg() {
  ...
  const user: iMyInput = MyInput({
    type: "text",
    placeholder: "Enter name....",
    name: "User name",
    cn: "myinput-auth",
  });
  const password: iMyInput = MyInput({
    type: "password",
    placeholder: "Enter password....",
    name: "Password",
    cn: "myinput-auth",
  });
  const repeatpassword: iMyInput = MyInput({
    type: "password",
  ...
  const arr: any[] = [
    { id: 1, name: user },
    { id: 2, name: password },
    { id: 3, name: repeatpassword },
  ];
  const messagerUser = MyMessager({
    title: "такой пользователь существует!!!",
    style: { color: "red" },
  });
      ...
      if (findUser === undefined) {
        ...
        if (findId === undefined) {
          localStorage.setItem(
            ...
            JSON.stringify([
              ...server,
              { name: user.value, password: password.value, id: Math.floor(id), favorites: [] },
            ]),
          );
          dispatch({
            type: "auth",
  ...
  return (
    <>
      ...
      >
        {arr.map((item) => (
          <div key={item.id}>{item.name.input()}</div>
        ))}
